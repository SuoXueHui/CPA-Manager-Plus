...
## 数据流
Manager 账号列表请求携带 `grouped=true`，Controller 在分页前按 `provider + lower(trim(email))` 聚合；无完整邮箱时优先按 `provider + auth_index`，缺失凭证索引再回退物理账号 ID。Controller 返回代表账号字段、`merged`、`credential_count`、`credential_ids` 及聚合后的 `usage_windows`。不改变默认未分组 API 和容量决策逻辑。
