...
func Migrate(db *sql.DB) error {
	...
	statements := []string{
		...
		`create table if not exists auth_file_activity (
		...
		`create index if not exists idx_auth_file_activity_scope_file
			on auth_file_activity(scope_key, auth_file_name)`,
		`create index if not exists idx_auth_file_activity_scope_auth_index
			on auth_file_activity(scope_key, auth_index)`,
...
func ensureMonitoringIndexes(db *sql.DB) error {
	// These fallback indexes are created after the legacy snapshot-column
	// migration so an older database can upgrade safely in one restart. The
	// v1.11.12 derived monitoring path handles covered ranges, while these
	// indexes keep raw edge and recovery queries bounded.
	for _, statement := range []string{
		...
		if _, err := db.Exec(statement); err != nil {
			return fmt.Errorf("create monitoring fallback index: %w", err)
		}
	}
	return nil
}
